...
调用RegExp构造方法有一个好处是里面可以执行js代码，实现动态匹配
...
### 贪婪模式与非贪婪模式
  ...
  默认情况下是贪婪模式,数字3~6个，他会匹配到最大的数字个数，所以结果是X78,在数量词后面加上?后，就开启了非贪婪模式，那就是匹配最小的数字个数，结果就变成了XX78.

  那第三个是什么原因呢？当贪婪模式匹配字符串的时候字符个数已经不够最大次数的匹配的时候，会选择更小次数的匹配。开始它匹配了6个，最后又匹配了三个所以结果就是XX.
...
|test()|检索字符串中指定的值.返回true和false;影响lastIndex属性的值|

#### test()

检索字符串中指定的值.返回true和false;影响lastIndex属性的值

![pic5](js正则表达式/pic5.png)

可以看到，这里面第一次test返回的是true,第二次却变成了false,我们的正则并没有写错，其实这里面就是lastIndex在搞鬼，第一次匹配后，**lastIndex就变成了lastIndex+匹配字符的长度**，现在就是1.

![pic6](js正则表达式/pic6.png)

第二次再调用test的时候,从下标1向后匹配是匹配不到字符的，因此返回false,然后lastIndex初始化为0,下次test会再为true

#### exec()

检索字符串中指定的值.返回找到的值,并确定其位置，影响lastIndex属性的值，如果没有匹配的文本，则返回null
...
返回一个或者多个子串
与indexOf()方法类似,但是他返回匹配的值，而不是匹配字符所在的下标

#### search()

返回第一次出现匹配指定正则表达式子串的下标,若没有则返回-1,与indexOf类似，但是indexOf()不支持正则
